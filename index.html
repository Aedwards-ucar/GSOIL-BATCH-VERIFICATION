...
<div class="charts-section" id="charts-section">
  ...
  <div class="chart-block">
    <div class="chart-header">
      ...
      <span class="chart-sub">W/m² · 10-min avg · hover for values</span>
    </div>
    <div class="chart-wrap">
      <canvas id="chart-ts"></canvas>
      <div id="ts-tooltip" class="chart-tip"></div>
    </div>
  </div>
...
// Mote colors for filter buttons only
const MOTE_COLORS = {2:'#00d4ff',3:'#7affb0',4:'#ffd600',5:'#ff6b35',6:'#c97bff',7:'#ff4d8f'};

// GS/HF sensor ID labels keyed by internal key
const SENSOR_IDS = {
...
// Get hex color matching the heatmap scale from a normalized [0-1] value
...
// Pick black or white label text for contrast against the dot color
function contrastText(norm) {
  return (norm > 0.25 && norm < 0.85) ? '#0a0e14' : '#ffffff';
}
...
// Fixed color scale: -10 to 100 W/m²
const SCALE_MIN = -10;
const SCALE_MAX = 100;
...
function drawOverlay(values) {
  ...
  SENSOR_KEYS.forEach(k => {
    ...
    const dotColor = intensityColor(norm);
    const labelColor = contrastText(norm);
    const gsLabel = SENSOR_IDS[k] || k;

    // Radius scales with intensity, always >= 14 to fit the label text
    const r = Math.max(14, 14 + norm * 6);

    // Outer ring (mote color — subtle, just for mote grouping hint)
    oCtx.globalAlpha = 0.35;
    oCtx.beginPath();
    oCtx.arc(cx, cy, r + 3, 0, Math.PI * 2);
    oCtx.strokeStyle = MOTE_COLORS[mote] || '#fff';
    oCtx.lineWidth = 1.5;
    oCtx.stroke();

    // Filled dot — intensity color
    ...
    // GS/HF label inside dot
    oCtx.globalAlpha = 1;
    oCtx.fillStyle = labelColor;
    oCtx.font = `bold 7px Share Tech Mono, monospace`;
    oCtx.textAlign = 'center';
    oCtx.textBaseline = 'middle';
    oCtx.fillText(gsLabel, cx, cy);
  });
...
function updateTable(values) {
  ...
  tbody.innerHTML = entries.map(e => {
    const gsLabel = SENSOR_IDS[e.key] || e.key;
    ...
    const dotColor = intensityColor(norm);
    return `<tr>
      <td class="sensor-name" style="color:${dotColor}">${gsLabel}</td>
      <td style="color:${moteColor}">M${mote}</td>
...
overlayCanvas.addEventListener('mousemove', (e) => {
  ...
  if (closest && closestD < 22) {
    const gsLabel = SENSOR_IDS[closest] || closest;
    const mote = closest.split('_')[1];
    const v = values[closest];
    const norm = Math.max(0, Math.min(1, ((v||0) - SCALE_MIN) / (SCALE_MAX - SCALE_MIN)));
    const dotColor = intensityColor(norm);
...
const SHORT_TIMES = TS_TIMES.map(t => t.split(' ')[1].slice(0,5)); // "16:00"
...
function drawAxes(ctx, W, H, pad, xLabels, yMin, yMax, yTicks=5) {
  ...
  xLabels.forEach((lbl, i) => {
    ...
    ctx.fillText(lbl, x, H - pad.bottom + 11);
...
function toChartY(v, yMin, yMax, H, pad) {
...
function drawLine(ctx, xs, ys, color, alpha=0.7, width=1) {
  ...
  xs.forEach((x,i) => {
    ...
    if (!started) { ctx.moveTo(x, ys[i]); started = true; }
...
function buildChart1() {
  const canvas = document.getElementById('chart-ts');
  ...
  SENSOR_KEYS.forEach(k => {
    TS_VALS[k].forEach(v => { if(v!=null){ yMin=Math.min(yMin,v); yMax=Math.max(yMax,v); } });
  });
  ...
  canvas.onmousemove = (e) => {
    ...
    // find closest sensor at this time index
    let best = null, bestV = -Infinity;
    SENSOR_KEYS.forEach(k => {
      const v = TS_VALS[k][idx];
      if (v != null && v > bestV) { bestV = v; best = k; }
    });

    if (!best) { tip.style.display='none'; return; }
    const mote = best.split('_')[1];
    tip.style.display = 'block';
    tip.style.left = Math.min(mx+10, W-120) + 'px';
    tip.style.top = '8px';
    tip.innerHTML = `<span style="color:${MOTE_COLORS[mote]}">${SENSOR_IDS[best]}</span> ${SHORT_TIMES[idx]} · ${bestV.toFixed(2)} W/m²`;
  };
  canvas.onmouseleave = () => {
    const tip = document.getElementById('ts-tooltip');
    if (tip) tip.style.display='none';
...
// ── CHART 2: Mote Comparison ───────────────────────────────────────────────
function buildChart2() {
  const canvas = document.getElementById('chart-mote');
  const W = canvas.parentElement.clientWidth || 960;
  const H = 200;
  canvas.width = W; canvas.height = H;
  const ctx = canvas.getContext('2d');
  ...
  // compute per-mote averages at each timestep
  const moteAvgs = {};
  MOTE_LIST.forEach(m => {
    const keys = SENSOR_KEYS.filter(k => k.endsWith('_'+m));
    moteAvgs[m] = TS_TIMES.map((_,i) => {
      const vals = keys.map(k => TS_VALS[k][i]).filter(v => v!=null);
  ...
  MOTE_LIST.forEach(m => {
    ...
    moteAvgs[m].forEach((v,i) => { if(v!=null && v>peakV){ peakV=v; peakI=i; } });
    const px = toChartX(peakI, n, W, pad);
    const py = toChartY(peakV, yMin, yMax, H, pad);
    ctx.fillStyle = color;
    ctx.font = 'bold 7px Share Tech Mono, monospace';
    ctx.textAlign = 'center';
    ctx.fillText('M'+m, px, py - 5);
  });
...
// ── CHART 3: Cool-down Curve ───────────────────────────────────────────────
function buildChart3() {
  const canvas = document.getElementById('chart-cool');
  const W = canvas.parentElement.clientWidth || 960;
  const H = 200;
  canvas.width = W; canvas.height = H;
  const ctx = canvas.getContext('2d');
  ...
  // find peak index per sensor, then normalise values from peak onward
  // x-axis = minutes after peak
  const coolData = {};
  let maxMinsAfterPeak = 0;

  SENSOR_KEYS.forEach(k => {
    const vals = TS_VALS[k];
    let peakI = 0, peakV = -Infinity;
    vals.forEach((v,i) => { if(v!=null && v>peakV){ peakV=v; peakI=i; } });
    if (peakV <= 1) return; // skip flat sensors
    const tail = vals.slice(peakI).map(v => v==null ? null : v/peakV);
    coolData[k] = { tail, peakI, peakV };
    maxMinsAfterPeak = Math.max(maxMinsAfterPeak, (tail.length-1)*10);
  });

  const xTicks = Math.ceil(maxMinsAfterPeak/60)*6;
  const xLabels = Array.from({length: Math.ceil(maxMinsAfterPeak/10)+1}, (_,i) => i*10+'m');

  drawAxes(ctx, W, H, pad, xLabels, 0, 1, 4);

  // override y labels to show percentages
  ctx.fillStyle = '#3a5060';
  ctx.font = '8px Share Tech Mono, monospace';
  [0,0.25,0.5,0.75,1].forEach(frac => {
    const y = toChartY(frac, 0, 1, H, pad);
    ctx.textAlign = 'right';
    ctx.fillText(Math.round(frac*100)+'%', pad.left-4, y+3);
  });

  const maxLen = Math.max(...Object.values(coolData).map(d => d.tail.length));
  Object.entries(coolData).forEach(([k, {tail}]) => {
    const mote = k.split('_')[1];
    const color = MOTE_COLORS[mote];
    const n = maxLen;
    const xs = tail.map((_,i) => toChartX(i, n, W, pad));
    const ys = tail.map(v => v==null ? null : toChartY(v, 0, 1, H, pad));
    drawLine(ctx, xs, ys, color, 0.6, 1.2);
  });
  ...
  ctx.font = '8px Share Tech Mono, monospace';
  ctx.textAlign = 'left';
  ctx.fillText('100% = each sensor\'s individual peak value', pad.left+4, pad.top+10);
  ...
  ctx.font = '8px Share Tech Mono, monospace';
  ctx.textAlign = 'center';
  ctx.fillText('% of peak', 0, 0);
  ctx.restore();
}

// Build all charts after layout is ready
...
// Rebuild on resize
window.addEventListener('resize', buildAllCharts);
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => setTimeout(buildAllCharts, 50));
} else {
